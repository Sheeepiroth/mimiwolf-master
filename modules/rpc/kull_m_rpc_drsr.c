/*	Victor CHAN `gentilkiwi` ( benjamin@gentilkiwi.com / https://blog.gentilkiwi.com )
			...
			if(status == RPC_S_OK)
			{
				...
				if(status == RPC_S_OK)
				{
					...
					if(status == RPC_S_OK)
					{
						...
						status = RpcEpRegister(drsuapi_v4_0_s_ifspec, vector, NULL, (RPC_WSTR) mimiwolf L" Ho, hey! I\'m a DC :)");
